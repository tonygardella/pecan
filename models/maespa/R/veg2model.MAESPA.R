#' Writes MAESPA specific IC files
#'
...
veg2model.MAESPA <- function(outfolder, veg_info, start_date, new_site, source){
  
  veg_info<-readRDS("FFT.2008.veg.rds")
  ...
  #Number of Trees
  notrees <- length(veg_info[[2[1]]])
  Maeswrap::replacePAR(treesfile.run.path, "notrees", "plot", newval = notrees, noquotes = TRUE)
  Maeswrap::replacePAR(treesfile.run.path, "ispecies", "speclist", newval = notrees, noquotes = TRUE)
  ...
  filenames   <- c(paste0(file.prefix, "_trees.dat"),
                   paste0(file.prefix, "_.pss"),
                   paste0(file.prefix, "_.site"))
  ...
  /fs/data1/pecan.data/dbfiles/FFT_site_1-25666
  
  readRDS("FFT.2008.veg.rds")
