...
read.register <- function(register.xml, con){
  ...
  if(is.null(register$type)){logger.error("Registration file type is not defined.")
  }else if(register$type == "model"){
    
    if(is.null(register$update)){ 
      logger.error("Model met update is not defined") 
    }else{
      invisible(register)
      }
    
  }else if(register$type == "met"){
